...
export function VariantSelector({ lang, product: { meta, variants } }) {
  ...
  optNames.forEach((name) => (options[name] = Array.from(options[name])));

  return optNames.map((name, i) => (
    <dl dir="auto" className="mb-4" key={name}>
      ...
      <dd className="flex flex-wrap gap-3">
        {options[name].map((value) => {
          ...
          const availableForSale = selectedVariant?.quantity > 0;
          const isDisabled = !optNames[i + 1] && !availableForSale;
